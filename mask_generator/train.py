...
weight_fn = os.path.join("weights", "checkpoint_pavements_{}.pth.tar".format(datetime.now().strftime("%Y%m%d_%H%M%S")))
...
def get_data(count, train_picture_path, train_mask_path, decal=0):
    ...
    train_picture_name = [os.path.join(train_picture_path, name) for name in sorted(os.listdir(train_picture_path))[decal:count+decal]]
    train_mask_name = [os.path.join(train_mask_path, name) for name in sorted(os.listdir(train_mask_path))[decal:count+decal]]

    images = torch.zeros((count, 3, to_even(image_height/shrink_factor), to_even(image_width/shrink_factor)))
    for i in range(count):
        images[i] = downscale(read_image(train_picture_name[i]).float() / 255.0, factor=shrink_factor)[:, :3, :, :]

    masks = torch.zeros((count, 2) + images.shape[2:])
    for i in range(count):
        raw_mask = downscale(read_image(train_mask_name[i]).float(), factor=shrink_factor)
